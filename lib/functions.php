 ...
 */
function password_ash($password_clear,$enc_type) {
	if (DEBUG_ENABLED && (($fargs=func_get_args())||$fargs='NOARGS'))
		debug_log('Entered (%%)',1,0,__FILE__,__LINE__,__METHOD__,$fargs);
...
//			$a[$key] = preg_replace('/\\\([0-9A-Fa-f]{2})/e',"''.chr(hexdec('\\1')).''",$rdn);
			$a[$key] = preg_replace_callback('/\\\([0-9A-Fa-f]{2})/', function($m){ return ''.chr(hexdec($m)).'';}, $rdn);
		};
		return $a;

	} else {
//		return preg_replace('/\\\([0-9A-Fa-f]{2})/e',"''.chr(hexdec('\\1')).''",$dn);
		return preg_replace_callback('/\\\([0-9A-Fa-f]{2})/', function($m) { return ''.chr(hexdec($m)).'';}, $dn);
	}
}
